...
import { cacheKey, readCache, writeCache, startOfDay, endOfDay, addHours, addDays } from '../utils';
...
export const useTodosStore = create<TodosState>((set, get) => {
  ...
  if (cachedUid) {
    initialTodos = readCache<Todo[]>(cacheKey(cachedUid, CACHE_KEY)) ?? [];
  }
  ...
  return {
    ...
    init: (uid: string) => {
      const unsub = subscribeToAllTodos(uid, (todos) => {
        ...
        writeCache(cacheKey(uid, CACHE_KEY), todos);
      });
      return unsub;
    },
