...
import { sandboxCacheKey, readCache, writeCache, startOfDay } from '../utils';
...
export const useLogsStore = create<LogsState>((set, get) => {
  ...
  let initialLogs: Log[] = [];
  if (cachedUid) {
    initialLogs = readCache<Log[]>(sandboxCacheKey(cachedUid, CACHE_KEY)) ?? [];
  }
  ...
  return {
    ...
    init: (uid: string) => {
      const unsub = subscribeToRecentLogs(uid, (logs) => {
        ...
        writeCache(sandboxCacheKey(uid, CACHE_KEY), logs);
      });
      return unsub;
    },
