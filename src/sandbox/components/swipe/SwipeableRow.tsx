...
export default function SwipeableRow({
  ...
  if (disabled) {
    return <div className="sb-swipe-container">{children}</div>;
  }

  return (
    <div className="sb-swipe-container">
      {/* Right-swipe: reveal left-side actions */}
      {rightActions.length > 0 && (
        <div className="sb-swipe-actions sb-swipe-actions--right">
          {rightActions.map((action) => (
            <button
              key={action.label}
              type="button"
              className={`sb-swipe-action ${action.className}`}
              onClick={() => { action.onTrigger(); springBack(); }}
            >
              {action.icon && <span className="sb-swipe-action-icon">{action.icon}</span>}
      ...
      {leftActions.length > 0 && (
        <div className="sb-swipe-actions sb-swipe-actions--left">
